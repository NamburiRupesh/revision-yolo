...
def parse_model(d, ch, verbose=True):
    ...
    # Define base and repeat modules
    base_modules = frozenset({
        ...
        C2fAttn, C3, C3TR, C3Ghost, torch.nn.ConvTranspose2d, DWConvTranspose2d, C3x, RepC3, PSA,
        SCDown, C2fCIB, A2C2f
    })
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        # Replace first two Conv layers with GhostConv
        if "nn." in m:
            m = getattr(torch.nn, m[3:])
        elif "torchvision.ops." in m:
            m = getattr(__import__("torchvision").ops, m[16:])
        else:
            if m == "Conv" and i in (0, 1):  # Only replace first two Conv layers
                m = GhostConv
            else:
                m = globals()[m]
        ...
        elif m in frozenset({Detect, WorldDetect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB,
